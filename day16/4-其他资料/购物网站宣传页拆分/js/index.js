 ...
 * @Last Modified time: 2017-10-11 12:31:57
 */
$(function() {
    // 调用fullpage插件
    $('#fullpage').fullpage({
    ...
    function section3Animate() {
        //让沙发变大从上往下掉下来
        $('.section3-sofa').show().animate({
            'bottom': '250px',
            'width': '207px'
        }, 1500, function() {
            //让右侧选择尺寸的第二个图片渐渐显示
            $('.section3-icon2').animate({
                'opacity': 1
            }, 500);
            //让右侧的白色购物车渐渐的显示
            $('.section3-cart2').animate({
                'opacity': 1
            }, 500);
        });
    }
    ...
    function section4Animate() {
        // 沙发从上往下掉下来到购物车里面
        $('.section4-cart img').eq(0).show().animate({
            'bottom': '200px'
    ...
    function section8Animate() {
        ...
        $('.section8-again').on('click',function () {
           ...
           //把所有图片的行内样式清空  动画效果就清除了
           $('.section img').attr('style',''); 
           //把所有div的行内样式清空 动画效果也清除了
           $('.section div').attr('style',''); 
        });
    }
});
